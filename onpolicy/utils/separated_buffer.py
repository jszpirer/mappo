...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space):
        ...
        self.share_obs = [[[] for _ in range(self.n_rollout_threads)] for _ in range(self.episode_length + 1)]
        self.obs = [[[] for _ in range(self.n_rollout_threads)] for _ in range(self.episode_length + 1)]
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        share_obs = concatenated_share_obs.reshape(-1, *self.share_obs[0].shape[1:])
        concatenated_obs = np.concatenate(self.obs[:-1], axis=0)
        obs = concatenated_obs.reshape(-1, *self.obs[0].shape[1:])
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        if len(self.share_obs.shape) == 4:
            share_obs = self.share_obs[:-1].transpose(1, 0, 2, 3).reshape(-1, *self.share_obs.shape[2:])
            obs = self.obs[:-1].transpose(1, 0, 2, 3).reshape(-1, *self.obs.shape[2:])
        elif len(self.share_obs.shape) > 3:
            print("Tests")
            print(self.share_obs.shape)
            print(self.share_obs[:-1].shape)
