...
class Scenario(BaseScenario):
    ...
    def observation(self, agent, world):
        ...
        entity_pos_0 = np.zeros((world.grid_resolution, world.grid_resolution))
        entity_pos_1 = np.zeros((world.grid_resolution, world.grid_resolution))
        entity_pos_2 = np.zeros((world.grid_resolution, world.grid_resolution))
        i = 0
        for entity in world.landmarks:  # world.entities:
            if np.linalg.norm(entity.state.p_pos - agent.state.p_pos) <= range_observation:
                ...
                if i == 0:
                    entity_pos_0[round(coef*distance[0]) + scale][round(coef*distance[1]) + scale] = 1
                elif i == 1:
                    entity_pos_1[round(coef*distance[0]) + scale][round(coef*distance[1]) + scale] = 1
                else:
                    entity_pos_2[round(coef*distance[0]) + scale][round(coef*distance[1]) + scale] = 1
            i += 1
        ...
        observations = np.concatenate((np.array(vel), np.array(goal_color), np.array(comm_padded), np.array(entity_pos_0), np.array(entity_pos_1), np.array(entity_pos_2)), axis=0)
        return observations
