...
class Scenario(BaseScenario):
    ...
    def observation(self, agent, world):
        ...
        for other in world.agents:
            ...
            i += 1
        obs_pos = np.zeros((2, world.num_obstacles + 4))
        vertical_wall = np.zeros((world.num_obstacles + 4 + 1))
        vertical_wall[0] = 5
        horizontal_wall = np.zeros((world.num_obstacles + 4 + 1))
        horizontal_wall[0] = 5
        i = 0
        for obs in world.obstacles:
            distance = np.linalg.norm(obs.state.p_pos - agent.state.p_pos)
            dist = obs.state.p_pos - agent.state.p_pos
            obs_pos[0][i] = round(coef*dist[0]) + scale
            obs_pos[1][i] = round(coef*dist[1]) + scale
            i += 1
        ...
        observations[:] = [agent.state.p_vel, agent.state.p_pos, other_pos, vertical_wall, horizontal_wall, obs_pos]
        return observations
