...
class Scenario(BaseScenario):
    ...
    def observation(self, agent, world):
        ...
        velocity[0] = np.cos(agent.direction_init)*agent.state.p_vel[0] + np.sin(agent.direction_init)*agent.state.p_vel[1]
        velocity[1] = -np.sin(agent.direction_init)*agent.state.p_vel[0] + np.cos(agent.direction_init)*agent.state.p_vel[1]
        
        observations = np.empty([2], dtype=object)
        observations[:] = [velocity, other_pos]
        return observations
