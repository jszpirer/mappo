...
class Scenario(BaseScenario):
    def make_world(self, args):
        ...
        # Walls
        side_length = 2.2414
        self.num_points = int(side_length/((2 * 7.86)/world.grid_resolution)) + 1
        n_sides = 12
    ...
    def is_occluded_for_camera(self, agent_pos, target_pos, world, target_dist):
        for blocker in world.agents:
            if np.array_equal(blocker.state.p_pos, target_pos):
                continue
            blocker_vec = blocker.state.p_pos - agent_pos
            blocker_dist = np.linalg.norm(blocker_vec)
            if blocker_dist < target_dist:
                blocker_angle = np.arctan2(blocker_vec[0], blocker_vec[1])
                blocker_width_angle = 2 * np.arctan(blocker.size / blocker_dist)
                target_angle = np.arctan2(target_pos[0] - agent_pos[0], target_pos[1] - agent_pos[1])
                angle_diff = (target_angle - blocker_angle + np.pi) % (2 * np.pi) - np.pi
                if abs(angle_diff) < blocker_width_angle / 2:
                    return True
        return False

    def is_occluded_for_lidar(self, agent_pos, target_pos, world, target_dist):
        for blocker in world.agents:
            if np.array_equal(blocker.state.p_pos, target_pos):
                continue
            blocker_vec = blocker.state.p_pos - agent_pos
            blocker_dist = np.linalg.norm(blocker_vec)
            if blocker_dist < target_dist:
                # Distance orthogonale entre les vecteurs
                rel_vec = target_pos - agent_pos
                cross_dist = np.linalg.norm(np.cross(rel_vec, blocker_vec)) / np.linalg.norm(rel_vec)
                if cross_dist < 0.0629:
                    # Comparison with the size of the lidar
                    return True
        return False
    ...
    def observation(self, agent, world):
        ...
        for other in world.agents:
            ...
            if cam_min <= dist <= cam_max and angle_to_other <= cam_fov / 2:
                if not self.is_occluded_for_camera(agent_pos, other.state.p_pos, world, dist):
                    camera = True
                else:
                    occluded = True

            if lidar_min <= dist <= lidar_max:
                if not self.is_occluded_for_lidar(agent_pos, other.state.p_pos, world, dist):
                    lidar = True
                else: 
                    occluded = True
            ...
            for point in wall_points:
                ...
                for blocker in world.agents:
                    ...
                    if np.linalg.norm(blocker_vec) < dist and np.dot(blocker_vec, rel_pos) > 0:
                        if np.linalg.norm(np.cross(rel_pos, blocker_vec)) / dist < 0.0629:
                            occluded = True
                            break
